...
from .services import (_from_random, _retrieve_recommendations_and_sort_by, _validate)
...
def _from_collaborative_filtering(enrolled_subjects):
    ...
    print("==========content-based filtering begin==========")
    ...
    ratings_matrix = ratings.pivot_table(index=['subject_id'], columns=['student_id'], values='rating').reset_index(drop=True)
    ...
    recommendations = _retrieve_recommendations_and_sort_by(
        recommended_subjects)
    print("==========content-based filtering end==========")
    return recommendations
